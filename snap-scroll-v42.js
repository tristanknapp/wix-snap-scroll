console.log("[Wix Snap Scroll v4.0.2] file executing");

(() => {
  "use strict";

  const VERSION = "Wix Snap Scroll v4.0.2";
  ...
  const CONFIG = {
    ...
    desktop: {
      ...
      /*
       * Ignore snap points this close to the current scroll
       * position when choosing the next directional target.
       */
  ...
  const CLEANUP_KEY =
    "__WIX_SNAP_SCROLL_V402_CLEANUP__";
  ...
  function buildSnapPoints() {
    ...
    /*
     * Preserve configured page order.
     */
    snapPoints =
      points.sort((a, b) => {
        if (
  ...
  /*
   * Select the physically next point below or above the current
   * page position.
   *
   * This avoids stale snap counts after Wix menu navigation and
   * prevents short sections from being skipped.
   */
  ...
  function getFaqState(
    ...
    /*
     * The page scroll position is within the FAQ's usable
     * scrolling range.
     */
    ...
    /*
     * This supports Wix menu-anchor offsets without allowing
     * small partial FAQ visibility to activate too early.
     */
    const centerInside =
      viewportCenter >=
        faq.top &&
  ...
  function desktopMoveOneSection(direction) {
    ...
    /*
     * When genuinely inside the FAQ, leave through its configured
     * neighboring section.
     */
    if (faq.active) {
      desktopMoveToIndex(
    ...
    /*
     * Otherwise choose the physically next section relative to
     * the real scroll position. This prevents the section before
     * the FAQ—or any other short section—from being skipped.
     */
    const targetIndex =
      findDirectionalSnapIndex(
        direction
  ...
  function onDesktopWheel(event) {
    ...
    if (faq.active) {
      ...
      /*
       * Normal scrolling remains enabled while there is still
       * content inside the FAQ in the requested direction.
       */
