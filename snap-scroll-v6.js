console.log("[Wix Snap Scroll v4.1.1] file executing");

(() => {
  "use strict";

  const VERSION = "Wix Snap Scroll v4.1.1";

  const CONFIG = {
    mobileBreakpoint: 767,

    /*
     * Snap scrolling runs ONLY on these paths.
     *
     * For testing:
     *   /home-1
     *
     * Later, when this becomes your real homepage:
     *   replace "/home-1" with "/"
     */
    allowedPaths: [
      "/home-1"
    ...
    /*
     * Wix can navigate between pages without a full reload.
     * Check the pathname regularly.
     */
  ...
  const CLEANUP_KEY =
    "__WIX_SNAP_SCROLL_V411_CLEANUP__";
  ...
  function updatePageActivation(
    ...
    if (shouldRun) {
      if (!engineActive) {
        ...
        /*
         * Wix may have replaced the page DOM during routing.
         * Rediscover the correct document and sections.
         */
        if (!findPageDocument()) {
          engineActive = false;

          log(
            "Allowed page found, but Wix sections are not ready yet"
          );
  ...
  function checkRouteChange() {
    ...
    /*
     * Give Wix a short moment to swap the new page DOM.
     */
    window.setTimeout(
      () => {
        if (
          ...
          /*
           * Re-find the Wix page document after navigation.
           */
          findPageDocument();
        }
  ...
  function getSectionMode(element) {
    ...
    /*
     * Everything else is a normal snap section.
     */
    return {
      mode: "SNAP",
      parts: 1
  ...
  function cleanupEngine() {
    ...
    /*
     * Extra safety:
     * make sure touch blocking is gone on non-scroller pages.
     */
    setControlledTouch(
      false
    );
  ...
  function initialize() {
    ...
    /*
     * Install the route watcher regardless of what page we
     * initially land on.
     */
    installRouteWatcher();

    /*
     * If this isn't an allowed page, do NOT install scrolling.
     */
    if (
      !shouldRunOnCurrentPage()
    ) {
